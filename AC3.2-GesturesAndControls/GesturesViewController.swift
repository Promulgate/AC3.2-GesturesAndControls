...
class GesturesViewController: UIViewController {
  var correctRedColorValue = 0.5
  var correctGreenColorValue = 0.5
  var correctBlueColorValue = 0.5
  var wrongRedColorValue = 0.5
  var wrongGreenColorValue = 0.5
  var wrongBlueColorValue = 0.5
  var keepingScore = true
  var stepperWin = 0
  var gameOver = false
  ...
  func isCorrect(_ correct: Bool) {
    ...
    if correct {
      ...
      if keepingScore {
        self.currentScore += 1
        if currentScore == stepperWin {
          actionToPerformLabel.text = "You Win! \n Press Save to start a new game"
          gameOver = true
        }
      }
    }
    else {
      self.view.backgroundColor = UIColor(red: CGFloat(self.wrongRedColorValue), green: CGFloat(self.wrongGreenColorValue), blue: CGFloat(self.wrongBlueColorValue), alpha: 1.0)
      if keepingScore {
        self.currentScore = 0
      }
    }
